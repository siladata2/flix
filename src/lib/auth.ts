...
/** Redirects to /login (or / if signed in but under-privileged) unless the
 *  caller's role meets `minimum`. Use at the top of every admin page/layout
 *  and every mutating Route Handler. */
export async function requireRole(minimum: UserRole) {
  const session = await getSessionProfile();
  if (!session) redirect('/login?redirectTo=/admin');
  if (ROLE_RANK[session.profile.role] < ROLE_RANK[minimum]) redirect('/');
  return session;
}
